...
df <- read_csv(here('data','merged_data_raw.csv'))
#income <- read_csv(paste0(path, inc_file))
income <- read_csv(here('data','income','income_clean.csv'))
#unemployment <- read_csv(paste0(path, ur_file))
unemployment <- read_csv(here('data','unemployment','unemployment_clean.csv'))
#interest <- read_csv(paste0(path, ir_file))
interest <- read_csv(here('data','interest','interest_clean.csv'))
#population <- read_csv(paste0(path, pop_file))
population <- read_csv(here('data','population','population_redo.csv'))
...
fault_df <- read_csv(here('data','fault_lines','faults_scored.csv'))
...
library(Boruta)
#lm.all <- lm(house_price~., data = train_set)
boruta_output <- Boruta(house_price~., data = train_set, doTrace=2)
...
#final model
boruta_output <- Boruta(log(house_price)~.-income+log(income)-population+log(population), data = train_quake, doTrace=2)
boruta_signif <- names(boruta_output$finalDecision[boruta_output$finalDecision %in% c("Confirmed", "Tentative")])  # collect Confirmed and Tentative variables
print(boruta_signif)  # significant variables
par(mfrow = c(1,1)) 
plot(boruta_output, cex.axis=.7, las=2, xlab="", main="Variable Importance")  # plot variable importance
